...
[`semhelpinghands`](https://sfcheung.github.io/semhelpinghands/). They
are used for generating plots of distribution of bootstrap estimates, as
suggested by Rousselet et al. (2021), to check if there is anything
unusual in the bootstrap distribution, such as bootstrap estimates that
are unusually extreme compared to other estimates.
...
#> 7    ab :=     a*b    ab 0.197 0.049 3.987  0.000    0.106    0.296
#> 8 total := c+(a*b) total 0.205 0.081 2.520  0.012    0.040    0.361
```
...
To plot the distribution of bootstrap estimates of free a parameter (not
user-defined parameter, i.e., not `ab`), call
[`plot_boot()`](https://sfcheung.github.io/semhelpinghands/reference/plot_boot.md).
The following is a sample call to plot the bootstrap estimates of the
`b` path:
...
- The argument `param` should be the name of the parameter to be
  plotted, as appeared in a call to
  [`coef()`](https://rdrr.io/r/stats/coef.html).

- The argument `standardized` is required. It indicates whether the
  bootstrap estimates from the standardized solution are to be plotted.

The argument `qq_dot_size` is optional, for controlling the size of the
points in the normal QQ-plot.

This is the plot

![Parameter: b](figure/fig1-1.png)

Parameter: b

The plot is similar to that of the output of
[`boot::boot()`](https://rdrr.io/pkg/boot/man/boot.html). The left panel
is a histogram with:

- A red dotted line to represent the point estimate of the parameter
  (the estimate of `b` in the `lavaan` output in this example).

- A kernel density plot (blue line) of the distribution.

The right panel is a normal QQ-plot generated by
[`qqnorm()`](https://rdrr.io/r/stats/qqnorm.html) and
[`qqline()`](https://rdrr.io/r/stats/qqnorm.html). If the distribution
is normal, the points should lie on the line. Deviation from a normal
...
Only the bootstrap estimates of free parameters are stored by `lavaan`.
...
Once computed and stored,
[`plot_boot()`](https://sfcheung.github.io/semhelpinghands/reference/plot_boot.md)
can be used again. For example, to plot the distribution of `ab`, the
...
![User-Parameter: ab](figure/fig2-1.png)

User-Parameter: ab

The plot suggests that, as expected, the sampling distribution of the
indirect effect is not normal (positively skewed).
...
Once computed and stored,
[`plot_boot()`](https://sfcheung.github.io/semhelpinghands/reference/plot_boot.md)
can be used again. For example, to plot the distribution of the
standardized indirect effect, just set `param` to `"ab"` *and* set
...
![User-Parameter: ab (Standardized)](figure/fig3-1.png)

User-Parameter: ab (Standardized)

Note that it is not unusual for a parameter to have a nonnormal sampling
distribution in the standardized solution. For example, this is the plot
...
![Y ~ M: Standardized](figure/fig4-1.png)

Y ~ M: Standardized

Compared to the previous plot of `b` in the original (unstandardized)
solution, the distribution is nonnormal (negatively skewed).

## Customizing the Plots

Users can customize a lot of aspects of the plot, such as the color of
the lines, the number of bars, and the size of the dots in the normal
QQ-plot. Please refer to the help page of
[`plot_boot()`](https://sfcheung.github.io/semhelpinghands/reference/plot_boot.md)
